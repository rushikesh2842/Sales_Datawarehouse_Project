...
manifest = {
    "entries": [
        {"url": f"s3://{bucket_name}/{key}", "mandatory": True}
        for key in files
    ]
}

# Write manifest file back to S3
manifest_key = f"{prefix.rstrip('/')}/manifest.json"
s3.put_object(Body=json.dumps(manifest), Bucket=bucket_name, Key=manifest_key)

print(f"Manifest created at s3://{bucket_name}/{manifest_key}")
